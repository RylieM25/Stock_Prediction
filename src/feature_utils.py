...
def extract_features():
    ...
    stk_tickers = ['PG', 'KO', 'COST','PEP','WMT']
    ...
    Y = np.log(stk_data.loc[:, ('Adj Close', 'PG')]).diff(return_period).shift(-return_period)
    Y.name = Y.name[-1]+'_Future'
    
    X1 = np.log(stk_data.loc[:, ('Adj Close', ('KO', 'COST','PEP','WMT'))]).diff(return_period)
